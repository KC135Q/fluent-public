...
const app = express();
const PORT = 8080;
const ipUrl = 'https://raw.githubusercontent.com/KC135Q/ip-lists/master/addresses.txt';
const ipUrl2 = "https://raw.githubusercontent.com/KC135Q/ip-lists/master/addresses3.txt";
...
function startIpUpdate() {
  ...
  setTimeout(async () => {
    ...
    await fluentFile.getCurrentFile(ipUrl2)
    if (fluentFile.lastUpdated !== fluentFile.currentDate) {
      fluentFile.getAddressesToRemove().forEach((address) => {
        console.log(`Remove ${address}`);
        fluentTree.removeIpAddress(address);
